...
La fuente vive en GitHub público. `.github/workflows/ci.yml` ejecuta las pruebas y el empaquetador; `curseforge.yml` prepara releases y puede subirlas cuando el proyecto de autor esté conectado. `tools/package.py` sigue siendo el único constructor del ZIP: lista seis archivos, valida XML/TOC y contrasta sus bytes. `tools/release.py` obtiene versión/canal/changelog, prepara metadatos y sube ese ZIP mediante la API de autores; no reconstruye al publicar. El logo de `branding/` queda fuera del paquete.
